...
export default async function handler(req, res) {
  ...
  const url = req.url || '';
  const isAdminAuth = url.endsWith('/auth') || url.includes('/auth');
  const isOverview = url.endsWith('/overview') || url.includes('/overview');
  const isRedemptions = url.endsWith('/redemptions') || url.includes('/redemptions');
  const isTransactions = url.endsWith('/transactions') || url.includes('/transactions');
  const isUsers = url.endsWith('/users') || url.includes('/users');

  const sql = getDb();

  // 1. Admin Auth (POST /api/admin/auth) - checks password without requiring founder payload yet
  if (isAdminAuth) {
  ...
  // All other endpoints require isFounder verification
  if (!isFounder(req)) return res.status(403).json({ error: 'Forbidden' });

  // 2. Admin Overview (GET /api/admin/overview)
  if (isOverview) {
    ...
    try {
      const [stats] = await sql`
        SELECT
          ...
          COUNT(CASE WHEN t.bonus_claimed=true THEN 1 END) AS bonus_claims
      ...
      const topMerchants = await sql`
        ...
        FROM transactions GROUP BY merchant ORDER BY tx_count DESC LIMIT 10`;

      const topSpenders = await sql`
        SELECT u.name,u.email,u.occupation,
               COUNT(t.id) AS tx_count,COALESCE(SUM(t.amount),0) AS total_spent,
               u.coin_balance,u.base_coins,u.bonus_coins
        FROM users u LEFT JOIN transactions t ON t.user_id=u.id
        GROUP BY u.id ORDER BY total_spent DESC NULLS LAST LIMIT 10`;

      const trend = await sql`
        SELECT DATE(created_at) AS day,COUNT(*) AS tx_count,
               SUM(amount) AS total_amount,SUM(total_coins) AS coins_issued
        FROM transactions WHERE created_at>NOW()-INTERVAL '14 days'
        GROUP BY DATE(created_at) ORDER BY day ASC`;

      const appBreakdown = await sql`
        SELECT payment_app,COUNT(*) AS tx_count,SUM(amount) AS total_amount
        FROM transactions WHERE payment_app IS NOT NULL
        GROUP BY payment_app ORDER BY tx_count DESC`;

      return res.status(200).json({
        stats:{
          total_users:          Number(stats.total_users),
          new_users_7d:         Number(stats.new_users_7d),
          total_transactions:   Number(stats.total_transactions),
          total_spend:          Number(stats.total_spend),
          total_base_coins:     Number(stats.total_base_coins),
          total_bonus_coins:    Number(stats.total_bonus_coins),
          total_coins_issued:   Number(stats.total_coins_issued),
          avg_tx_value:         Number(Number(stats.avg_tx_value).toFixed(2)),
          avg_spend_per_user:   Number(Number(stats.avg_spend_per_user).toFixed(2)),
          purchase_submissions: Number(stats.purchase_submissions),
          bonus_claims:         Number(stats.bonus_claims),
        },
  ...
  // 3. Admin Redemptions (GET /api/admin/redemptions)
  if (isRedemptions) {
    if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
    try {
      const rows = await sql`
        SELECT r.*,u.name AS user_name,u.occupation
        FROM reward_redemptions r JOIN users u ON u.id=r.user_id
        ORDER BY r.redeemed_at DESC`;
      return res.status(200).json(rows.map(r=>({
        id:r.id, user_name:r.user_name, user_email:r.user_email, occupation:r.occupation,
        brand:r.brand, label:r.label, code:r.code, coins_spent:Number(r.coins_spent), redeemed_at:r.redeemed_at,
      })));
    } catch(err) {
      return res.status(500).json({ error: err.message });
    }
  ...
  // 4. Admin Transactions (GET /api/admin/transactions)
  if (isTransactions) {
    if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
    try {
      const rows = await sql`
      ...
      return res.status(200).json(rows.map(t=>({
        ...
        purchase_note:t.purchase_note, bonus_claimed:t.bonus_claimed, created_at:t.created_at,
      })));
    } catch(err) {
      return res.status(500).json({ error: err.message });
    }
  ...
  // 5. Admin Users Management (GET/PATCH/DELETE /api/admin/users)
  if (isUsers) {
    try {
      if (req.method === 'GET') {
