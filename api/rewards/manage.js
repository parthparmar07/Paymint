...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (action==='update') await sql`UPDATE rewards SET brand=${newBrand||brand},label=${newLabel||label},cost_coins=${Number(newCost)} WHERE brand=${brand} AND label=${label}`;
    if (action==='delete') await sql`DELETE FROM rewards WHERE brand=${brand} AND label=${label}`;
