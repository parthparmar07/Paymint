...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (Number(userRows[0].coin_balance) < cost) return res.status(400).json({ error:`Insufficient coins` });
    const codeRows = await sql`UPDATE rewards SET stock=0,active=false WHERE id=(SELECT id FROM rewards WHERE brand=${brand} AND label=${label} AND active=true AND stock>0 ORDER BY created_at ASC LIMIT 1) RETURNING code`;
    ...
    await sql`INSERT INTO reward_redemptions (user_id,user_email,brand,label,code,coins_spent) VALUES (${payload.userId},${payload.email},${brand},${label},${codeRows[0].code},${cost})`;
    const fresh = await sql`SELECT coin_balance FROM users WHERE id=${payload.userId}`;
    return res.status(200).json({ code:codeRows[0].code, coins_spent:cost, coin_balance:Number(fresh[0].coin_balance) });
