...
export default async function handler(req, res) {
  ...
  try {
    ...
    await sql`UPDATE transactions SET purchase_note=${note.trim()},bonus_coins=${bonus},total_coins=base_coins+${bonus},bonus_claimed=${eligible} WHERE id=${transactionId}`;
    if (eligible && bonus > 0) await sql`UPDATE users SET coin_balance=coin_balance+${bonus},bonus_coins=bonus_coins+${bonus},updated_at=NOW() WHERE id=${payload.userId}`;
    const uRows = await sql`SELECT coin_balance,base_coins,bonus_coins FROM users WHERE id=${payload.userId}`;
    return res.status(200).json({ bonus_awarded:eligible&&bonus>0, bonus_coins:bonus, coin_balance:Number(uRows[0].coin_balance), base_coins:Number(uRows[0].base_coins), bonus_coins_total:Number(uRows[0].bonus_coins) });
