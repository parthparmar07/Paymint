...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      stats:{
        total_users:Number(stats.total_users), new_users_7d:Number(stats.new_users_7d),
        total_transactions:Number(stats.total_transactions), total_spend:Number(stats.total_spend),
        total_base_coins:Number(stats.total_base_coins), total_bonus_coins:Number(stats.total_bonus_coins),
        ...
        purchase_submissions:Number(stats.purchase_submissions), bonus_claims:Number(stats.bonus_claims),
      },
    ...
    return res.status(500).json({ error: 'Failed to load overview: '+err.message });
  }
}
