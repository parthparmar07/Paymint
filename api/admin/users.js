...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return res.status(200).json(users.map(u=>({
        ...
        total_base_coins:Number(u.total_base_coins), total_bonus_coins:Number(u.total_bonus_coins),
  ...
  if (req.method === 'PATCH') {
    try {
      ...
      console.error('[/api/admin/users PATCH]', err.message);
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      console.error('[/api/admin/users DELETE]', err.message);
